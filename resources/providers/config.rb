...
action :add do
  begin
    ...
    groups.each do |group|
      bindings = group['bindings'].keys.map(&:to_i).flatten
      name = group['name']
      ...
      %w(unicode.map classification.config reference.config sid-msg.map). each do |rfile|
      ...
      group['bindings'].keys.map(&:to_i).sort.map { |x| "#{x}" }.each do |id|
        vgroup         = group['bindings'][id].to_hash.clone
        vgroup_name    = vgroup['name'].nil? ? 'default' : vgroup['name'].to_s
        vgroup['name'] = vgroup_name
        vgroup['id']   = id

        if !(vgroup['vlan_objects'] && !vgroup['vlan_objects'].empty?) && !(vgroup['network_objects'] && !vgroup['network_objects'].empty?)
      ...
      [
        { files: "/etc/snort/#{group['instances_group']}/snort-binding-*", regex: %r{/snort-binding-(\d+)$} },
      ].each do |x|
        Dir.glob(x[:files]).each do |f|
          match = f.match(x[:regex])
          if match && !bindings.include?(match[1].to_i)
            if ::File.directory?(f)
              directory f do
                recursive true
                action :delete
      ...
      [
        { files: "/var/log/snort/#{group['instances_group']}/instance-*", regex: %r{/instance-(\d+)$} },
      ].each do |x|
        Dir.glob(x[:files]).each do |f|
          match = f.match(x[:regex])
          next unless match && match[1].to_i < group['cpu_list'].size && ((Time.now - ::File.stat(f).mtime) > 3600 * 24 * 7)

          # This directory can be deleted
          if ::File.exist?("#{f}/archive")
